...
import { treeDiffer } from "../../src/treeDiffer";
 ...
 */
class DomTreeNode extends treeDiffer.TreeNode<Node> {
     ...
     */
    isEqual(otherNode: typeof this) {
        if (this.node.nodeType === Node.TEXT_NODE) {
            return (
                otherNode.node.nodeType === Node.TEXT_NODE &&
                otherNode.node.textContent === this.node.textContent
            );
        } else if (
            otherNode.node.nodeType === Node.ELEMENT_NODE &&
            this.node.nodeType === Node.ELEMENT_NODE
        ) {
            return (otherNode.node as Element).tagName === (this.node as Element).tagName;
        } else {
			return false
		}
    }
     ...
     */
    // abstract getOriginalNodeChildren() : TreeNode<NodeType>[];
    getOriginalNodeChildren(): Node[] {
        let i: number,
            ilen: number,
            childNode: Node,
            children: Node[] = [],
    ...
    static showExampleDiff() {
        let i: number,
            ...
            tree1,
            tree2,
            treeInput1 = document.getElementsByClassName(
                "treeInput1"
            )[0] as HTMLTextAreaElement,
            treeInput2 = document.getElementsByClassName(
                "treeInput2"
            )[0] as HTMLTextAreaElement,
         ...
         */
        function addClassToNode(node: Node, className: string) {
            let span;
        ...
        tree1 = new treeDiffer.Tree<Node>(root1, DomTreeNode, {});
        tree2 = new treeDiffer.Tree<Node>(root2, DomTreeNode, {});

        // STEP 4

        //diff = new treeDiffer.Differ(tree1, tree2).diff();
        diff = new treeDiffer.Differ(tree1, tree2).transactions[
            tree1.orderedNodes.length - 1
        ][tree2.orderedNodes.length - 1];
